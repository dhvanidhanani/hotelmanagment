						...
						<form id="contactForm">
							<div class="row">
								...
								<div class="col-md-6">
									...
									<div class="col-md-12">
										<div class="form-group">
											<input type="text" placeholder="Your Numbar" name="table_id" class="form-control" required data-error="Please enter your Numbar" value="table id :<?php echo $_GET['id']; ?>">
											<div class="help-block with-errors"></div>
										</div> 
									</div>                              
								</div>
...
if(isset($_POST['book'])) {
    ...
    $table_id =  $_POST['table_id']; // Retrieve the table_id from the form

    $formatted_time = date("H:i:s", strtotime($reservation_time));

    mysqli_query($con,"insert into reservation(name, email, phone, reservation_date, reservation_time,num_persons, table_id) values('$name', '$email', '$phone', '$reservation_date', '$formatted_time', '$num_persons', '$table_id')") or die(mysqli_error($con));
    echo "<script>
        alert('Your table book successfully...');
        window.location.href='table.php';
        </script>";

    // $query = "INSERT INTO reservation (name, email, phone, reservation_date, reservation_time, num_persons, table_id) VALUES ('$name', '$email', '$phone', '$reservation_date', '$formatted_time', '$num_persons', '$table_id')";
...
//     $reservation_datetime = date('Y-m-d H:i:s', strtotime("$reservation_date $reservation_time"));
//     echo "Reservation Datetime: $reservation_datetime"; 

//     // SQL query to insert data into database
//     $query = "INSERT INTO reservation (name, email, phone, reservation_datetime, num_persons) 
//               VALUES ('$name', '$email', '$phone', '$reservation_datetime', '$num_persons')";
